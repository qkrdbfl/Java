...
public class Application2 {
    public static void main(String[] args) {
        //강제 형변환의 규칙에 대해 이해//
        //바꾸려는 자료형의 캐스트 연산자를 이용하여 형변환을 한다.
        // 예) (바꿀 자료형)  값; // (double) 10; // < 이렇게 앞에 써주기.

        //큰 자료형에서 작은 자료형응로 변경할 경우 강제 형변환이 필요.
        /* 1. 정수 끼리의 강제 형변환 */
        long lnum = 8;
        //int inum = lnum; //데이터 손실 가능성을 컴파일러가 알려준다. (에러남)
        int inum = (int) lnum; //변경하려는 자료형을 명시하여 강제 형변환을 해야 함
        short snum = (short) inum; //(short) <강제 명시.
        byte bnum = (byte) snum;
        /* 2. 실수 끼리의 강제 형변환 */
        double dnum = 8.0;
        //float fnum = dnum; //데이터 손실 가능성을 컴파일러가 알려준다. (에러남)
        float fnum = (float) dnum; //(fliat)  강제 명시.
        System.out.println(fnum);


        //실수를 정수로 변경할 경유 강제 형변환이 필요하다//
        float fnum2 = 4.0f;
